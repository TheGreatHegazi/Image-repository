...
export default {
  ...
  methods: {
    async refreshImages(){
      await axios.get('http://127.0.0.1:1919/images/public').then((result) => {
        this.images =  result.data.imgNames.map(n => 'http://127.0.0.1:1919/images/'+n);
      }).catch((err) => {
        console.log(err)
      });
    },
    ...
    async addImg() {
      ...
      await axios.post('http://127.0.0.1:1919/images/public/bulk',
        formdata, {
          headers: {
